...
func New(ID string, opts *Options) *Identicon {
	...
	rect := image.Rectangle{
		Min: image.Point{0, 0},
		Max: image.Point{opts.ImageSize + 2*opts.Margin, opts.ImageSize + 2*opts.Margin},
	}
...
func (ic *Identicon) drawTile(xTile, yTile int) {
	...
	xStart := (xTile * (ic.Options.ImageSize / tilesPerDimension)) + ic.Options.Margin
	xEnd := xStart + (ic.Options.ImageSize / tilesPerDimension)

	yStart := (yTile * (ic.Options.ImageSize / tilesPerDimension)) + ic.Options.Margin
